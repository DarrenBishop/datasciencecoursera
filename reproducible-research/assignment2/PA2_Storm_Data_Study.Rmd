---
title: "Severe Weather Impact on The US Economy and Public Health"
...
```{r Event Type Normalisation Procedure, cache=TRUE}
...
```{r Damage Normalisation Procedure, caceh=TRUE}
...
With this data cleaning strategy, the number of event-types is reduced to `r no_clean_event_types`.  
...
```{r, fig.cap = "Top 10 Harmful (Fatality/Injury) Storm Events"}
...
```{r, fig.cap = "Top 10 Costly (Economic) Storm Events"}
...
Drilling down into the data further, the vast majority of Tornado cases (`r signif(100 * no1_harmful$Injuries / no1_harmful$Harmful, 3)`%) were injuries; Excessive-Heat caused approximately `r signif(no2_harmful$Fatalities - no1_harmful$Fatalities, 1)` more fatalities.

### Economy

Figure. 2 shows that **Thunderstorms** consistently have the greatest impact on the US economy over the period, causing approximately **$2.38 billion** worth of damage, `r signif(100 * no2_costly$value / no1_costly$value, 3)`% more than the next most costly, **Flash-Flood**, causing approximately $1.41 billion worth of damage.  
